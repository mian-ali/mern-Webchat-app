...
const initSocket = (server, corsOptions) => {
  ...
  io.on('connection', socket => {
    ...
    socket.on('ENTER_CHAT_ROOM', roomData => {
      ...
      //add new
      socket.join(roomId)
      // Someone other than yourself receives the message
    ...
    socket.on('ROOM_CREATED', ({ name, creator, invitedUser }) => {
      invitedUser.forEach(invitedUser => {
        ...
        // The invited person will be notified only when they are online
        if (socketId) {
          socket.to(socketId).emit('INVITED_TO_ROOM', { message: `${creator} You have been added ${name} chatroom`})
        }
      })
    })
  })
